...
@njit(cache=True)
def prepare_gauss(
    energy: np.ndarray, xanes: np.ndarray
...
def convolve_gaussian(
    energy: np.ndarray, xanes: np.ndarray, E_cut: float, sigma_gauss: float, vibration: float = 0.0
) -> np.ndarray:
    Ef, de, nenerg, nj, Xa = prepare_gauss(energy=energy, xanes=xanes)
